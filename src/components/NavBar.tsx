...
function NavBar (props: NavBarProps) {
    ...
    const handleClickMobileCta = () => {
        let panel: HTMLElement | null = document.querySelector("#mobile-cta-panel-container");
    ...
    return <nav>
        <div className="nav-icon nav-item">Hello<span className="nav-sub-icon">Movies</span></div>
        <div className="nav-pages nav-item">
            <div id="mobile-cta-panel-container">
                <div id="mobile-cta-panel">
                    <BackspaceIcon id="mobile-cta-close" onClick={handleClickMobileCtaClose}/>
                    {props.pages.map((item: string, index: number) => {
                        return <div key={index} className={props.curPage === item ? "page-title-highlight" : "page-title-normal"} onClick={() => handleClickTitle(item)}
                            >{item}</div>
                    })}
                </div>
            </div>
